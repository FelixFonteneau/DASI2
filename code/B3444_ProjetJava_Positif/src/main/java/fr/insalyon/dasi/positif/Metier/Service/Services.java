...
public class Services {
    ...
    public Medium recupererMedium(Long id) {
        MediumDAO mDao = new MediumDAO();
        Medium m;
        try {
            JpaUtil.creerEntityManager();
            JpaUtil.ouvrirTransaction();
            m = mDao.findMedium(id);
            JpaUtil.validerTransaction();
        } catch (RollbackException ex) {
            m = null;
            DebugLogger.log("La recherche du client a échoué");
    ...
    public Consultation getConsultEnCours(Employe e){
        EmployeDAO cDao = new EmployeDAO();
        Consultation consult = null;
        try {
                JpaUtil.creerEntityManager();
                JpaUtil.ouvrirTransaction();
                List<Consultation> consults = cDao.findConsultationOfEmploye(e);
    ...
    public List<Consultation> genererHistoriqueClient(Client c) {
        ...
        for (Consultation co1 : lCon) {
            if (co1.getClient().getAdresseMail().equals(c.getAdresseMail())) {
                lHistorique.add(co1);
